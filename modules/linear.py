...
class Linear:
    ...
    def forward(self, x):
        ...
        N = np.shape(x)[0]
        x_reshaped = x.copy()
        x_reshaped = x_reshaped.reshape(N,self.in_dim)
        out = np.dot(x_reshaped, self.weight) + self.bias
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        self.cache = x, x_reshaped
        return out
    ...
    def backward(self, dout):
        ...
        '''
        x, x_reshaped = self.cache
        #############################################################################
        # TODO: Implement the convolution backward pass.                            #
        #############################################################################
        #self.dx = np.dot(dout,self.weight.T).reshape(-1,x.shape[1], x.shape[2])
